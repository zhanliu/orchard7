...
class ShippingAddressModel
{
    ...
    public function addShippingAddress($customer_id, $address_id)
    {
        ...
        $sql = "INSERT INTO shipping_address (customer_id, address_id, is_primary, created_time, updated_time) ";
        $sql.= "VALUES (:customer_id, :address_id, :is_primary,:created_time, :updated_time)";
    ...
    {
        $sql = "SELECT a.province, a.city, a.district, a.address1, a.address2, a.id, sa.is_primary, a.id, sa.id as shipping_id ";
...
//        echo $sql;
    ...
    public function setDefaultShippingAddress($shipping_address_id, $customer_id) {
        ...
        $sql = "update shipping_address set is_primary = 0 where customer_id =" . $customer_id . " and id != ";
        $sql.= $shipping_address_id;
        $query = $this->db->prepare($sql);
        $query->execute();

        $sql = "update shipping_address set is_primary = 1 where customer_id =" . $customer_id . " and id = ";
        $sql.= $shipping_address_id;
        $query = $this->db->prepare($sql);
        $query->execute();
    }
